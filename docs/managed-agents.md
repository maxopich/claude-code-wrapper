...
**Deliberately.** An encryption key that has to sit on the same disk as its ciphertext, readable by the same account, stops a casual grep and buys a key-management surface. What is done instead: the tree is 0700 and every copied file is written at exactly 0600 (`Cebab-6fax.43.2`) — uniformly, credential file or not. The copy used to keep an ordinary file's owner bits (`entry.mode & 0o700`) so its exec bit survived, while a `pathLooksSensitive` file was forced to 0600; that was two mechanisms encoding opposite intents. A managed agent's tree is config and source, read by an interpreter and never invoked, so nothing in it needs the exec bit — and `hardenDataDir` sweeps this tree to owner-only like any other data-dir path, so an exec exception would be a hole in exactly the sweep that exists to have none. The preflight still NAMES the credential files (via `pathLooksSensitive`, exported from `shared/src/redact.ts` and reused rather than restated), paths only, since the predicate opens nothing; and a `chmod` that fails is reported as `permissions_unenforced` rather than swallowed. Note the ordering of importance: the 0700 TREE is what keeps other accounts out, and the per-file modes are defence in depth behind it.
