...
- **OAuth credential hygiene on the CLI's own stored login.** Cebab uses the user's existing `claude` CLI subscription and never reads the credential itself — the SDK-spawned CLI resolves it. Where that login lives is the CLI's choice and is platform-dependent: `~/.claude/.credentials.json` on Linux and Windows, the Keychain (service `Claude Code-credentials`) on macOS, where that file does not exist at all. Compromise of the stored login is a local user-account concern outside Cebab's threat model.
...
The interesting property of Cebab is **runtime trust posture**: no bus tool call is ever gated on a human. Bus turns run `permissionMode: 'default'` with a `canUseTool` that auto-approves every tool except `AskUserQuestion`. A **trusted** participant also loads its project's own `.claude/settings*.json`, so its hooks execute on every hop; an **untrusted** participant runs `settingSources: ['user']` and loads none of it (`Cebab-6fax.21.1`) — Trust decides that per participant project, exactly as it does for a single-agent session. Under that posture, a malicious transitive npm `postinstall` script is direct RCE on an operator's machine. So the supply-chain surface (anything that lets attacker-controlled code land in `node_modules/` or in a CI workflow) carries higher severity than for a typical dev tool.

There is one structural exception, and its narrowness is the point: the **orchestrator** runs `toolPolicy: 'delegate-only'`, which is not an auto-approval at all but a default-deny. The built-in file/shell/analysis tools are stripped from its context via `disallowedTools`, and `canUseTool` independently denies anything that is not `bus_send` or `AskUserQuestion`. Read that second clause narrowly: `canUseTool` is **not** a universal gate — `permissions.allow` in any loaded settings file, and the CLI's own in-cwd read heuristic, both settle calls before the callback is consulted ([docs/safety-and-security.md](docs/safety-and-security.md) carries the measurement). Stripping the tool from the model's context is the half that holds unconditionally, which is why the `disallowedTools` list has to track the SDK's own tool catalogue rather than a hand-written copy of it. It applies to the orchestrator alone: every worker and every chain participant runs the auto-approve posture described above, which is why the paragraph above stands as the summary of Cebab's posture rather than this one.
