...
**Per-launch WS auth tokens**. At server boot Cebab generates a 32-byte hex token at `~/.cebab/auth-token` (mode 0600). The WS upgrade requires it as `?token=`; the browser fetches it from `/auth-token` (Origin+Host gated, same allow-list as the WS). This closes browser-tab CSWSH (a cross-origin tab can't pass the Origin gate to fetch the token) and cross-uid local attackers (mode 0600). It does NOT fully close the bus-worker hijack path: a worker pane runs under `bypassPermissions` as the operator's uid, so it can read the token from disk OR via `GET /auth-token` (empty-Origin branch returns the token to non-browser local clients). With the F2 orchestrator source allowlist + F6 `BUS_AGENT_NAME` shape regex in place (`bus/orchestrator.ts:handleEvent` and `bus/scripts/bus-send-msg.sh`), a token-holding worker's attack surface reduces to direct WS control-plane abuse — primarily `set_trusted` flipping a future session's Trust state. Operators should reset Trust state on any project after running a prompt-injection-prone multi-agent session in it. Same-uid isolation (Unix socket + `SO_PEERCRED`, or XPC with entitlements) is v2 work. A residual exists even after both filters: a worker can still impersonate another _known_ worker by setting `BUS_AGENT_NAME=<other-worker-slug>` — slug is valid and a participant; closing this requires a Cebab-as-arbiter redesign (Unix-socket-mediated bus where Cebab stamps `source` from tmux pane identity).
