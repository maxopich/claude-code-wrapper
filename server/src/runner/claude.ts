 ...
 * socket.
 *
 * `CLAUDE_CODE_API_KEY_FILE_DESCRIPTOR` WAS MISSING UNTIL `Cebab-6fax.23`. It
 ...
 * same exposure through a file rather than an env var; Cebab cannot scrub a
 * setting out of the operator's own `~/.claude/settings.json` (it writes
 * nothing there), so that half is SURFACED in the authority panel instead —
 * `detectApiKeyHelpers` in `repo/project_authority.ts`.
 *
...
export const SCRUBBED_ENV_VAR_NAMES: ReadonlyArray<string> = [
  ...
  // named fd and it overrides OAuth just as an inline key would (Cebab-6fax.23).
